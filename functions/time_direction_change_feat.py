...
def rxd_time_gap_direction_change_feat(train, test):
    # 这个函数内的特征都使用按时间排序后的数据集
    train_time_sort, test_time_sort = sort_by_time(train, test)

    # 取方向缺失次数
    train_1 = train_time_sort[['TERMINALNO', 'TRIP_ID', 'DIRECTION']].groupby(['TERMINALNO', 'TRIP_ID'],
                                                                              as_index=False).agg(
        fun_direction_none)
    test_1 = test_time_sort[['TERMINALNO', 'TRIP_ID', 'DIRECTION']].groupby(['TERMINALNO', 'TRIP_ID'],
                                                                            as_index=False).agg(
        fun_direction_none)
    ...
    # 取组内时间差距
    train_2 = train_time_sort[['TERMINALNO', 'TRIP_ID', 'TIME']].groupby(['TERMINALNO', 'TRIP_ID'],
                                                                         as_index=False).agg(
        lambda arr: (arr.iloc[-1] - arr.iloc[0]).seconds)
    test_2 = test_time_sort[['TERMINALNO', 'TRIP_ID', 'TIME']].groupby(['TERMINALNO', 'TRIP_ID'],
                                                                       as_index=False).agg(
        lambda arr: (arr.iloc[-1] - arr.iloc[0]).seconds)
