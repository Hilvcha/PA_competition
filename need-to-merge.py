...
#train_data=pd.read_csv('PINGAN-2018-train_demo.csv',usecols=['TERMINALNO','TIME','TRIP_ID','CALLSTATE','DIRECTION','Y'],nrows=35000)
#test_data=pd.read_csv('PINGAN-2018-train_demo.csv',usecols=['TERMINALNO','TIME','TRIP_ID','CALLSTATE','DIRECTION'],nrows=20000)
def fun(arr):
    return arr.iloc[0]
def get_label(df):
...
def direction_change_feat(train,test):
    train_1 = train[['TERMINALNO', 'TRIP_ID', 'DIRECTION']].groupby(['TERMINALNO', 'TRIP_ID'], as_index=False).agg(fun_direction_none)
    test_1 = test[['TERMINALNO', 'TRIP_ID', 'DIRECTION']].groupby(['TERMINALNO', 'TRIP_ID'], as_index=False).agg(fun_direction_none)
    ...
    train=train[['TERMINALNO','TRIP_ID','DIRECTION']].groupby(['TERMINALNO','TRIP_ID'],as_index=False).agg(fun_direction)
    test=test[['TERMINALNO', 'TRIP_ID', 'DIRECTION']].groupby(['TERMINALNO', 'TRIP_ID'], as_index=False).agg(fun_direction)
