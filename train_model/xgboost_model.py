...
def model_train():
    train,test,y_train,y_test=load_datasets()
    Id=test['TERMINALNO']
    del train['TERMINALNO']
    del test['TERMINALNO']
    x_train, x_val, y_train, y_val = train_test_split(train, y_train, test_size=0.2, random_state=100)
    ...
    Pred = -Pred
    Pred = list(Pred)
    submit_df=pd.merge(Id,Pred,on='TERMINALNO',how='left')
